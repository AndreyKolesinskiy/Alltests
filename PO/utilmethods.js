...
function UtilMethods () {
    ...
    that.selectRecord = function (mainFolder, subFolder, record) {
        ...
        return browser.wait(EC.visibilityOf(element(by.cssContainingText('.aciTreeText', mainFolder))),
            ...
            .then (function () {return browser.wait(EC.visibilityOf(element(by.cssContainingText('.aciTreeText', subFolder))),
                browser.params.visibilityWaitingTime.elementDrawing);
            })
            .then (function () {
                return element(by.cssContainingText('.aciTreeText', subFolder)).click();
            })
            .then (function () {
                return selectedItem.getAttribute('aria-expanded');
            })
            ...
            .then (function (){return browser.wait(EC.visibilityOf(element(by.cssContainingText('.aciTreeText', record))),
                browser.params.visibilityWaitingTime.elementDrawing);
            })
    ...
    that.selectMenuAndSubmenu = function (menu, subMenu) {
        return element(by.partialLinkText(menu)).$('.glyphicon').getAttribute('class')
            .then (function (string) {var arr = string.split(' ');
            ...
            .then (function () {return browser.wait(EC.visibilityOf(element(by.partialLinkText(subMenu))),
                browser.params.visibilityWaitingTime.elementDrawing);
            })
            .then (function(){
